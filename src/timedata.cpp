...
void AddTimeData(const CNetAddr& ip, int64_t nOffsetSample)
{
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        // Note: this comparison is written without abs64() because negating
        // std::numeric_limits<int64_t>::min() is undefined behaviour; a hostile
        // peer that supplies INT64_MIN as its time offset could otherwise
        // bypass the clamp entirely and drive our adjusted time to an extreme.
